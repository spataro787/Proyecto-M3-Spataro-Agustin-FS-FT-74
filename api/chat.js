module.exports = async function handler(req, res){
  ...
  const normalizedMessages = messages
    .map((msg) => {
      ...
      const author = role === 'user' ? 'user' : role === 'system' ? 'system' : 'model';
  ...
  if (normalizedMessages.length === 0) {
    return res.status(400).json({
      ok: false,
      status: 400,
      error: 'No hay texto válido en messages.'
    });
  }
  ...
  try {
    ...
    console.log('GEMINI_RESPONSE:', rawResponse);

    if (!response.ok) {
      const errorMessage = responseData?.error || responseData?.message || `Gemini API devolvió ${response.status}`;
...
function extractReply(data) {
  ...
  const outputContent = data?.output?.[0]?.content?.find((item) => item?.type === 'text')?.text;
  if (outputContent) {
